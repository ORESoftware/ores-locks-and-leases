...
pub fn maintenance_options_are_fail_closed_test() {
  ...
  let assert Error(error) = locks.validate_lease_maintenance_options(
    key(),
    opts,
...
pub fn renewal_preserves_fenced_identity_test() {
  let original = grant()
  let lease = lease_with_renew(fn(value, ttl_ms) {
...
pub fn changed_fencing_token_is_lost_lease_test() {
  let original = grant()
  let lease = lease_with_renew(fn(value, _) {
    Ok(locks.LeaseGrant(
      ..value,
