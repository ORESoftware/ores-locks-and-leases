    ...
    if (held) {
      throw wait
          ? LockError.timeout(
              ...
              opts.waitTimeout.inMilliseconds,
            )
...
void main() {
  ...
  test('lock-plan matrix', () {
    ...
    for (final c in matrix) {
      ...
      expect(
        plan(
          layers,
          PgScope.parse(c['pgScope'] as String),
          c['wait'] as bool,
        ).steps,
        expected,
  ...
  group('FiduciaLease', () {
      ...
      () async {
        final client = MockClient((request) async {
          expect(
            ...
            'https://fiducia.example/v1/locks/acquire',
          );
    ...
    test(
      'fails closed before sending credentials to public cleartext HTTP',
      () async {
        var called = false;
        final lease = FiduciaLease.bearer(
    ...
    test(
      'fails closed on fencing tokens outside exact JSON-number range',
      () async {
  ...
  group('withLease', () {
    ...
    test(
      'contention without wait, timeout with wait, lostLease on lapse',
      () async {
    ...
    test(
      'cleanup failure wins over work failure and retains both diagnostics',
      () async {
