...
fn same_owner_reacquisition_is_contention_not_reentrancy() {
    ...
    assert!(
        second.is_none(),
        "same owner must not imply recursive ownership"
    );
...
fn owner_identity_is_exact_unicode_not_normalized() {
    ...
    let error = lock
        .release()
        .expect_err("normalization-equivalent owner must not authenticate");
...
fn scoped_panic_keeps_panic_precedence_and_best_effort_releases() {
    ...
    assert!(
        result.is_err(),
        "panic must propagate rather than become a structured work error"
    );
    assert!(
        !path.exists(),
