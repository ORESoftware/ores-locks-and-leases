...
The official clients additionally cancel abandoned/ambiguous queued acquisition
identities and release a raced grant returned by cancellation. The local `Lease`
interface does not yet expose cancellation, so timeout cleanup remains bounded by
`wait_timeout_ms` and the grant TTL. Adding explicit cancel-on-timeout/abort is a
follow-up hardening item rather than silently pretending it is implemented.

## Runtime status

| Runtime | Integration | Status |
| --- | --- | --- |
| Go | local `net/http` adapter | Current acquire/renew/release wire shapes and stable request identity are enforced by tests. |
| TypeScript | local `fetch` adapter | Current acquire/renew/release wire shapes and stable request identity are enforced by tests. |
| Dart | local `package:http` adapter | Current acquire/renew/release wire shapes and stable request identity are implemented; CI exercises the package. |
| Gleam | local `gleam_httpc` adapter | Still requires the same queue-identity/current-wire migration before this audit is fully closed. |
