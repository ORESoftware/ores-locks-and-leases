...
exists on one workstation.  In particular, ordinary zed-pkg installs should
use the local path.  The zed-pkg convention is:

```text
$ZPKG_HOME/locks/
```

with `ZPKG_HOME` defaulting to `$HOME/.zpkg`.  A caller may create lock
identities such as `install.lock`, `refs.lock`, or
...
TypeScript/Node.js, Go, and Gleam.  It is intended for polyglot tools that need
a no-network single-host mutex and cannot share one native descriptor-lock API.
The protocol is deliberately tiny:

1. the caller chooses a lock-directory path and a non-empty per-acquisition
   owner token;
...
`$ZPKG_HOME` / `$ZPKG_HOME/locks` with user-private permissions where the
