...
BeamScale acquisition uses a stable `request_id` (from
`AcquireOptions.requestId`, or generated by the adapter). The durable actor
persists that request identity with the active lease. Repeating the same
holder/request pair replays the existing grant without extending its TTL or
minting a new sequence. The adapter therefore makes one safe recovery retry
after an ambiguous acquire transport failure. If that retry is also ambiguous,
it fails closed as `transport`.

After owner failover, an active lease is inherited by the new owner epoch and
is not replayable or renewable under the old token; the caller observes
contention until persisted expiry.
