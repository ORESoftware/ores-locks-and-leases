...
const fail = (message) => {
  throw new Error(`[polyglot-governance] ${message}`);
};
...
if (governance.schema !== 'ores.locks.polyglot-governance/v1') {
  fail('unsupported governance schema');
}
...
for (const participant of governance.participants) {
  ...
  if (sourceDirs.has(participant.sourceDir)) {
    fail(`duplicate participant sourceDir: ${participant.sourceDir}`);
  }

  if (boundaryNames.has(participant.boundaryLanguage)) {
    fail(`duplicate boundary language: ${participant.boundaryLanguage}`);
  }
...
if (
  boundaries.authorities?.typeSpec !== 'peer'
...
if (requiredBoundaries.size !== ids.size) {
  fail('required boundary count differs from governed participant count');
}

for (const language of boundaryNames) {
  if (!requiredBoundaries.has(language)) {
    fail(`governed language missing required bidirectional boundary: ${language}`);
  }
}

for (const language of requiredBoundaries) {
  if (!boundaryNames.has(language)) {
    fail(`required boundary missing governance participant: ${language}`);
  }
}
