...
async function loadBundle(name) {
  ...
  const verificationPath = `target/tjsv/${name}/consumer-verification.json`;
  try {
    ...
    const verificationPassed =
      ...
      Array.isArray(verification.declarationIds) &&
      verification.declarationIds.length === contractIr.declarations.length;
    ...
    return {
      ...
      consumerVerificationSha256: digest(verificationBytes),
...
const receipt = {
  ...
  runtimeBoundary: {
    ...
    canonicalContractIrVerificationRequired: true,
  },
  discrepancies,
